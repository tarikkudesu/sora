...
let	  colorSelector = 0;
const backgrounds = [`frame0`, `frame1`, `frame2`];
...
const updatePosition = (x, y) => {
	...
	shadow.style.left = `${x + 80}px`;
    shadow.style.top = `${y + 100}px`;
};
...
const update = () => {
    ...
    const isOverlapping = (element1, element2) => {
        ...
        const center1 = [rect1.left + rect1.width / 2, rect1.top + rect1.height / 2];
        const center2 = [rect2.left + rect2.width / 2, rect2.top + rect2.height / 2];
        const radius = Math.min(rect2.width, rect2.height) / 2;
        const distance = Math.pow(center2[0] - center1[0], 2) + Math.pow(center2[1] - center1[1], 2);
        return distance <= Math.pow(radius, 2);
    }

	const backOverlaping = (element1, element2) => {
		const rect1 = element1.getBoundingClientRect();
		const rect2 = element2.getBoundingClientRect();
	...
	if (isOverlapping(player, obs00) || isOverlapping(player, obs01) || isOverlapping(player, obs02) || isOverlapping(player, obs03) ||
		...
		isOverlapping(player, obs76) || isOverlapping(player, obs77) || isOverlapping(player, obs78) || isOverlapping(player, obs79) || !backOverlaping(player, background) ) {
        moveSpeed = 0.02;
        playerWidth -= playerWidth * 0.001;
        player.style.width = `${playerWidth}px`;
        player.style.height = `${playerWidth}px`;
        shadow.style.width = `${playerWidth}px`;
    ...
    indicator.innerText = `${Math.trunc(getDistanceBetweenElements(destEl, player))}`;
...
const startGame = () => {
    ...
    background.style.width = `${bgSize}px`;
    background.style.height = `${bgSize}px`;
    gameContainer.style.width = `${bgSize}px`;
    gameContainer.style.height = `${bgSize}px`;
    player.style.width = `${playerWidth}px`;
    player.style.height = `${playerWidth}px`;
    player.style.left = `${posP[0]}px`;
    player.style.top = `${posP[1]}px`;
    shadow.style.width = `${playerWidth}px`;
    shadow.style.height = `${playerWidth}px`;
    shadow.style.left = `${posP[0] + 80}px`;
    shadow.style.top = `${posP[1] + 100}px`;
